...
class Edrone():
	...
	def __init__(self):
		...
		self.Kp = [7.2,10.32,42,0]
		self.Ki = [0.024,0.016,0,0]
		self.Kd = [487.2,509.7,276.9,0]
	...
	* Example Call: if self.key_value == 70:
	...
	def pid(self):
		...
		fif = 1500
		path = 0
		self.setpoint = [self.drone_position[0],self.drone_position[1],self.drone_position[2]-10,0.0]
		self.errors[0] = self.setpoint[0] - self.drone_position[0]
		...
		#Holding till the path is formed

		while not self.points:
			#PID controller
			self.pid_calc()

		while path is not 4:	#Traversing all the paths computed
			for i in range (0,50):
				...
				while abs(self.errors[0])>0.95 or abs(self.errors[1])>0.95 or abs(self.errors[2])>0.95:
					#PID controller
					self.pid_calc()
					if time.time() - self.timeref > 0.3:
			...
			self.points = []
			while not self.points and path is not 4:	#Holding while the path is computed
				#PID controller
				self.pid_calc()
