...
class Edrone():
	...
	def __init__(self):
		...
		self.setpoint = [5.67,-1.91,33.0,0.0] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
		self.set_points = [[0,0,30],[0,0,29],[0,1,29],[0,2,29],[0,3,29],[1,3,29],[1,3,28],[1,3,27],[1,4,27],[2,4,27],[3,4,27]]
	...
	#------------------------------------------------------------------------------------------------------------------------
		...
		while abs(self.errors[0])>0.5 or abs(self.errors[1])>0.5 or abs(self.errors[2])>0.5 or abs(self.errors[3])>0.5:
			now = time.time()
		...
		#while path is not 4:		
			cordinates = []
			for i in range (0,10):
				cordinates = self.points[i]
			
				self.errors[0] = cordinates[0] - self.drone_position[0]
			
				self.errors[1] = cordinates[1] - self.drone_position[1]
			
				self.errors[2] = cordinates[2] - self.drone_position[2]
			
				self.errors[3] = cordinates[3] - self.drone_position[3]
