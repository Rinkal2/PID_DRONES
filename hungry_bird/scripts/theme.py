...
class Edrone():
	...
	def __init__(self):
		...
		self.lastTime = 0.0
		self.key_value = 0
		self.timeref = 0 #it is used to store time for case when whycon marker is not detected used in whycon_callback() and pid() function
	...
	* Example Call: while abs(self.errors[0])>0.8 or abs(self.errors[1])>0.8 or abs(self.errors[2])>0.8:
					#PID controller
					self.pid_calc()

	'''

	def pid_calc(self):	#PID controller
		now = time.time()
	...
	* Function Name: pid()
	* Input: none
	* Output: pid_calc() function is been called to handle the drone and traverse as per the conditions
	* Logic: pid_calc() function is called as per the conditions
	* Example Call: if self.key_value == 30:
			self.disarm()
			self.arm()
			self.pid()
	...
	def pid(self):
		...
		#zero = 0
		#fif = 1500
		path = 0	#used to send path number to be formed in vrep
		input_file = csv.DictReader(open("HB_Food_Config_Table.csv"))
		lis = []
		sum1 = 0
		for row in input_file:
			lis.append(row.values())
		for i in range(len(lis)-1):
			if lis[i][1] != 'ht':
				sum1 = sum1 + int(lis[i][1])
		total_path = (sum1*2+1)*2+1
		print total_path
		...
		while abs(self.errors[0])>0.5 or abs(self.errors[1])>0.5 or abs(self.errors[2])>0.5:	#Holding above the Start point
			
			self.pid_calc()	#calling PID controller

		path = path + 1				
		self.path_plan.publish(path)	#published path number in vrep to be formed


		while not self.points:	#Holding untill the path is formed
			
			self.pid_calc()	#calling PID controller

		while path is not total_path+1:	#Traversing all the paths computed
			for i in range(0,50):
				...
				while abs(self.errors[0])>0.8 or abs(self.errors[1])>0.8 or abs(self.errors[2])>0.8:	#Holding at one point then the other
					self.pid_calc()	#PID controller

			if self.key_value == 35 and path == 2:	#In case of repositioning
				path = path + 4
			path = path + 1				
			self.path_plan.publish(path)
			self.setpoint = self.points[49]
			self.points = []
			while not self.points and path is not total_path+1:	#Holding while the path is computed
