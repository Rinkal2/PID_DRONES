...
destination_flag = 0
way_points = [[0,0,30],[0,0,29],[0,1,29],[0,2,29],[0,3,29],[1,3,29],[1,3,28],[1,3,27],[2,4,27],[3,4,27]]
...
class DroneFly():
	...
	def pid_roll(self):
		...
		self.D_value_roll = 42000 * ( self.error_roll - self.DerivatorR)
	...
	def pid_pitch(self):
		...
		self.D_value_pitch = 42000 * ( self.error_pitch - self.DerivatorP)
	...
	def pid_throt(self):
		...
		self.P_value_throt = 4000 * self.error_throt
		self.D_value_throt = 60000 * ( self.error_throt - self.DerivatorT)
	...
	def get_pose(self,pose):
		...
		self.drone_z = pose.poses[0].position.z + 10
...
if __name__ == '__main__':
	while not rospy.is_shutdown():
		...
		for i in range(len(way_points)):
			temp.wp_x = way_points[i][0]
			temp.wp_y = way_points[i][1]
			temp.wp_z = way_points[i][2]
        		temp.position_hold()
        		print("Reached",temp.wp_x, temp.wp_y, temp.wp_z )
			#rospy.spin()
		destination_flag = 1
		temp.position_hold()	
