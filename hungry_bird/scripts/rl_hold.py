...
destination_flag = 0
way_points = [[0,0,30,0],[0,0,29,0],[0,1,29,0],[0,2,29,0],[0,3,29,0],[1,3,29,0],[1,3,28,0],[1,3,27,0],[2,4,27,0],[3,4,27,0]]
...
class Edrone():
	...
	* Example Call: rospy.Subscriber('whycon/poses', PoseArray, self.whycon_callback)
	...
	def whycon_callback(self,msg):
		...
		self.drone_position[2] = msg.poses[0].position.z+5
	...
	* Example Call: if self.key_value == 100:
	...
	def isThere(self):
		if(abs(self.drone_position[0] - self.setpoint[0]) < 0.5 and abs(self.drone_position[1] - self.setpoint[1])<0.5 and abs(self.drone_position[2] - self.setpoint[2])<0.5):
			return True
		else:
			return False
	...
	def pid(self):
		...
		while True:
			...
			if timechange >= self.sample_time:
				...
				self.lastTime = now
			if (self.isThere() and destination_flag==0):
				break
				
				print "PID",self.drone_position
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown():
		for i in range(len(way_points)):
			temp.setpoint[0] = way_points[i][0]
			temp.setpoint[1] = way_points[i][1]
			temp.setpoint[2] = way_points[i][2]
        		temp.pid()
        		print("Reached",temp.setpoint[0], temp.setpoint[1], temp.setpoint[2] )
			#rospy.spin()
		destination_flag = 1
		temp.pid()	
